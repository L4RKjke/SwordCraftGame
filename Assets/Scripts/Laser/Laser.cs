...
public class Laser : MonoBehaviour
{
    ...
    private Direction _sliceDirection;
    ...
    public UnityAction<Direction> Sliced;
    ...
    private void SliceSword()
    {
        ...
        {
            _sliceDirection = Direction.Left;    
        }

        else
        {
            _sliceDirection = Direction.Right;  
        }

        if (_sliceDirection == Direction.Left)
        {
            dirtController.RemoveLeftRocks(0);
            _slicer.transform.rotation = Quaternion.Euler(0, _rightSliceYAngel, _SliceZangel);
        }
            
        else if (_sliceDirection == Direction.Right)
        {
