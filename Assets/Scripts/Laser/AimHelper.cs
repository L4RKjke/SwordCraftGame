...
public class AimHelper : MonoBehaviour
{
    ...
    private void OnMissSword()
    {
        ...
        _model.EnableMove();
        _model.SetSpeed(platformMover.FastSpeed);
    }
    ...
    private void OnTriggerRay(Sword sword)
    {
        ...
        _swordMover.SetSpeed(platformMover.FastSpeed - 1);
        _redRay.SetActive(true);
        _model.transform.rotation = Quaternion.Euler(0, 0, 0);
        _redLine.DrawLine(_startLine.position, _endLine.position);
        _greenLine.enabled = true;
        _greenLine.DrawLine(_startLine.position, _endLine.position);
        _model.RotateSword(-1 * (180 / Mathf.PI) * Mathf.Atan((_endLine.position.x - _startLine.position.x) /
            (_endLine.position.z - _startLine.position.z)));
    }
}
